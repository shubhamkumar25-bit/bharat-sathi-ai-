...
export function AdminUserDetail() {
  ...
  async function updateUserStatus(nextStatus: 'active' | 'suspended' | 'inactive') {
    ...
    <div className="space-y-8">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  ...
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${userData.user.status === 'suspended' ? 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400'}`}>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => updateUserStatus(userData.user.status === 'suspended' ? 'active' : 'suspended')}
                disabled={actioning}
                className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
              >
                <ShieldAlert className="h-4 w-4" />
                {userData.user.status === 'suspended' ? 'Activate User' : 'Suspend User'}
              </button>
              <button
                onClick={deleteUser}
                disabled={actioning}
                className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-rose-900/40 dark:bg-rose-950/20 dark:text-rose-300"
              >
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          ...
          <p className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{userData?.featureCounts?.aiChats || 0}</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <p className="text-sm text-slate-500">Resumes Created</p>
          <p className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{userData?.featureCounts?.resumes || 0}</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <p className="text-sm text-slate-500">Schemes Explored</p>
          <p className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{userData?.featureCounts?.schemes || 0}</p>
        </div>
      </div>
